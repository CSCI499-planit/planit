...
def tag_untagged_places(places: list[PlaceRecord]) -> list[PlaceRecord]:
    ...
    untagged_places = [places[i] for i in untagged_indices]
    place_list = [
        {
            "name":       p.get("name", ""),
    ...
    if len(tags_list) != len(untagged_indices):
        logger.warning(
            "LLM tagger returned %d tag lists for %d places — skipping",
            len(tags_list), len(untagged_indices),
        )
        return places

    result_places = list(places)
    llm_tagged = 0
    for idx, tags_raw in zip(untagged_indices, tags_list):
    ...
    logger.info(
        ...
        llm_tagged, len(untagged_indices),
    )

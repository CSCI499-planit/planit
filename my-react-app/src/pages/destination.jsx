...
const PriceDots = ({ level }) => (
  <span className="price-dots">
    {[1,2,3].map(i => <span key={i} className={`price-dot ${i <= level ? 'filled' : ''}`} />)}
  </span>
)
...
export default function DestinationPage() {
  ...
  const [liked, setLiked] = useState(() => {
    const saved = userStorage.get('likedDestinations') || []
    return Object.fromEntries(saved.map(d => [d.id, true]))
  })

  const user = (() => { try { return JSON.parse(localStorage.getItem('userPreferences') || 'null') } catch { return null } })()

  const score = (d) => {
    if (!user) return d.rating
    let s = 0
    d.tags.forEach(tag => { if (user.preferred_tags?.includes(tag)) s += 2 })
  ...
  const filtered = destinations
    .filter(d => d.name.toLowerCase().includes(search.toLowerCase()) || d.country.toLowerCase().includes(search.toLowerCase()) || d.tags.some(t => t.includes(search.toLowerCase())))
    .filter(d => { if (!user) return true; return user.preferred_tags?.some(t => d.tags.includes(t)) || d.priceLevel <= Number(user.daily_budget_tier) || !user.party_type || d.audience?.includes(user.party_type) })
          ...
          <div key={d.id} className="dest-card">
            ...
            <div className="dest-card__body">
              <div className="dest-card__header">
                ...
                <div className="dest-card__rating"><StarIcon /> {d.rating}</div>
              </div>
              <div className="dest-card__meta">{d.country}&nbsp;·&nbsp;<PriceDots level={d.priceLevel} /></div>
              <p className="dest-card__desc">{d.description}</p>
              <div className="dest-card__tags">{d.tags.map(tag => <span key={tag} className="dest-tag">{tag}</span>)}</div>
              <div className="dest-actions">
                <button className={`icon-btn dislike ${disliked[d.id] ? 'active' : ''}`} onClick={() => handleDislike(d.id)} aria-label="Not for me"><XIcon active={disliked[d.id]} /></button>
                <button className={`icon-btn like ${liked[d.id] ? 'active' : ''}`} onClick={() => handleLike(d.id)} aria-label="Save">{liked[d.id] ? <HeartFilled /> : <HeartOutline />}</button>
              </div>
            </div>
          </div>
        ))}

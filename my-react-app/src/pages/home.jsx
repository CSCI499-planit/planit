  ...
  return (
    <div className="saved-card">
          ...
          <div className="saved-card__preview">
            {allStops.slice(0,3).map((s, i) => (
              <div key={i} className="saved-card__stop">
                <span className="saved-card__stop-dot" />{s.place?.name}
              </div>
            ))}
          </div>
          {allStops.length > 3 && <div className="saved-card__more">+{allStops.length - 3} more stops</div>}
        </>
      )}
            ...
            <div key={day.day} className="saved-detail-day">
              ...
              {day.stops.map((s, i) => (
                <div key={i} className="saved-detail-stop">
                  {s.place?.name}
                  {s.arrival_time && <span style={{ color: '#4a9bc5', marginLeft: 6 }}>{s.arrival_time}</span>}
                </div>
              ))}
            </div>
          ))}
      ...
      <div className="saved-card__footer">
        <span className="saved-card__days">{entry.tripDays} day{entry.tripDays !== 1 ? 's' : ''} · {allStops.length} stops</span>
  ...
  return (
    <div className="page">
      <div className="home__hero">
        ...
        <p>Ready for your next adventure? Let's make it unforgettable.</p>
        <div className="home__actions">
          <button className="btn btn--primary" onClick={() => navigate('/app/generate')}>Generate Itinerary</button>
          <button className="btn btn--secondary" onClick={() => navigate('/app/destination')}>Explore Destinations</button>
        </div>
      </div>

      <div className="stats-row">
        {[['Countries','20'],['Days Planned','40'],['Spent This Year','$2,300'],['Trips Done','5']].map(([label, val]) => (
      ...
      {likedDests.length === 0
        ? <p className="section-empty">No liked destinations yet — heart a place to save it here!</p>
              ...
              <div key={d.id} className="dest-chip">
                <div className="dest-chip__left">
                  <div>
                    ...
                    <div className="dest-chip__country">{d.country}</div>
                  </div>
                </div>
                <button className="dest-chip__remove" onClick={() => setConfirmDest(d)}>✕</button>
              </div>
            ))}
          </div>
      }
